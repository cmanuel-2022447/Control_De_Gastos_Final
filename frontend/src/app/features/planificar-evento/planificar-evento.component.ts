...
export class PlanificarEventoComponent implements OnInit {
  ...
  constructor(private eventosService: EventosService, private perfilService: PerfilService, private cdr: ChangeDetectorRef) {}

  ngOnInit(): void {
    this.perfilService.moneda$.subscribe((moneda) => { this.moneda = moneda; this.cdr.markForCheck(); });
    this.eventosService.eventos$.subscribe((eventos) => { this.eventos = eventos; this.cdr.markForCheck(); });
  }

  get presupuestoTotal(): number { return this.eventos.reduce((total, evento) => total + this.convertirDesdeGtq(Number(evento.presupuesto || 0)), 0); }
  get simboloMoneda(): string { return this.moneda === 'USD' ? '$' : 'Q'; }
  convertirDesdeGtq(monto: number): number { return this.moneda === 'USD' ? monto / this.tasaCambio : monto; }
  ...
  guardar(): void {
        ...
        if (!textoValido.test(this.formulario.nombre) || !textoValido.test(this.formulario.tipo) || (this.formulario.lugar && !textoValido.test(this.formulario.lugar)) || (this.formulario.invitados !== null && (!Number.isInteger(Number(this.formulario.invitados)) || Number(this.formulario.invitados) < 0)) || !/^\d{4}-\d{2}-\d{2}$/.test(this.formulario.fecha) || !/^(?:0|[1-9]\d*)(?:\.\d{1,3})?$/.test(this.formulario.presupuesto)) {
          this.errorMessage = 'El monto no puede tener más de 3 decimales.';
          return;
        }
    const payload = this.formulario as unknown as Omit<EventoData, 'id' | 'gastado' | 'restante' | 'estado'>;
    const request = this.editando === null ? this.eventosService.crear(payload) : this.eventosService.actualizar(this.editando, payload);
    request.subscribe({
  ...
  editar(evento: EventoData): void { this.editando = evento.id; this.formulario = { nombre: evento.nombre, tipo: evento.tipo, invitados: evento.invitados ?? null, lugar: evento.lugar || '', fecha: evento.fecha, presupuesto: String(evento.presupuesto) }; this.mostrarFormulario = true; this.successMessage = ''; }
